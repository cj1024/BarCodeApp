...
namespace CtripWP7.PNGExtention
{
    ...
    public class PngEncoder
    {
        ...
        /// </exception>
        public void Encode(WriteableBitmap image, Stream stream)
        {
            Debug.Assert(image != null, "image");
            Debug.Assert(stream != null, "stream");
        ...
        {
            if (_image.PixelWidth > 0 && _image.PixelHeight > 0)
            {
                var dpmX = (int)Math.Round(_image.PixelWidth * 39.3700787d);
                var dpmY = (int)Math.Round(_image.PixelHeight * 39.3700787d);
        ...
        private void WriteGammaChunk()
        {
            if (IsWritingGamma)
            {
                var gammeValue = (int)(Gamma * 100000f);

                var fourByteData = new byte[4];

                var size = BitConverter.GetBytes(gammeValue);
        ...
        {
            var pixels = _image.Pixels.GetColorByte();
            // Convert the pixel array to a new array for adding
            // the filter byte.
            // --------------------------------------------------
            var data = new byte[_image.PixelWidth * _image.PixelHeight * 4 + _image.PixelHeight];
            ...
            using (var tempStream = new MemoryStream())
            {
                ...
                {
                    // Write the length
                    var length = (ushort)((remainder < MaxBlockSize) ? remainder : MaxBlockSize);
                    tempStream.WriteByte((byte)(length == remainder ? 0x01 : 0x00));
                ...
                var zipData = new byte[tempStream.Length];
        ...
        private void WriteDataChunks()
        {
            ...
            try
            {
                ...
                using (var zStream = new DeflaterOutputStream(memoryStream))
                {
    ...
    public static class PngEncoderExtention
    {

        public static void SavePng(this WriteableBitmap image, Stream stream)
        {
